...
export default function Dashboard({ auth, stats, recentDocuments, unreadNotifications }) {
  ...
  return (
    <Layout title="Dasbor">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-white">Selamat Datang, {user.name}!</h1>
        <p className="text-gray-400 mt-1">Ringkasan aktivitas dokumen impor Anda.</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {Object.entries(stats || {}).map(([key, value]) => (
          <div key={key} className="bg-white/[0.02] border border-white/5 rounded-2xl p-5">
            <div className="text-sm text-gray-400 mb-1">{labels[key] || key}</div>
            <div className={`text-3xl font-bold ${STAT_COLORS[key] || 'text-white'}`}>{value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <Link href={LINK_MAP[user.role] || '/dashboard'} className="text-sm text-indigo-400 hover:text-indigo-300">Lihat semua</Link>
        </div>
        {(!recentDocuments || recentDocuments.length === 0) ? (
          ...
          <div className="space-y-3">
            {recentDocuments.map(doc => (
              <div key={doc.id} className="flex items-center justify-between py-3 border-b border-white/5 last:border-0">
                <div>
                  <div className="text-white font-medium">{doc.document_number}</div>
                  <div className="text-gray-500 text-sm">{doc.supplier?.company_name} &middot; {new Date(doc.document_date).toLocaleDateString('id-ID')}</div>
                </div>
