...
export default function DocumentDetail({ document }) {
  return (
    <Layout title={`Dokumen #${document.document_number}`}>
      <h1 className="text-2xl font-semibold text-white mb-6">Detail Dokumen</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6">
            ...
            <dl className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><dt className="text-gray-400">Status</dt><dd className="mt-1"><StatusBadge status={document.status} /></dd></div>
            ...
            {document.rejection_reason && (
              <div className="mt-4 p-3 bg-red-500/10 border border-red-500/20 rounded-xl">
                <div className="text-red-400 text-sm font-medium">Alasan Penolakan:</div>
                ...
                {document.files.map(file => (
                  <a key={file.id} href={`/files/${file.id}/download`} className="flex items-center justify-between p-3 bg-white/5 rounded-xl hover:bg-white/10 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-white text-sm">{file.file_name}</div>
                        <div className="text-gray-500 text-xs">{(file.file_size / 1024).toFixed(1)} KB</div>
                      </div>
                    </div>
              ...
              <div className="space-y-4">
                {document.audit_logs.map(log => (
                  <div key={log.id} className="relative pl-4 border-l-2 border-white/10">
                    <div className="text-white text-sm font-medium">{log.action.replace(/_/g, ' ')}</div>
