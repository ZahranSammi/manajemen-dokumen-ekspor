    ...
    <Layout title={`Dokumen #${document.document_number}`}>
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {canPrepare && (
            <button onClick={handlePrepare} className="px-4 py-2 bg-cyan-600 text-white rounded-lg text-sm font-medium hover:bg-cyan-700 transition-colors cursor-pointer">Siapkan Dokumen</button>
          )}
          {canArchive && (
            <button onClick={handleArchive} className="px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 transition-colors cursor-pointer">Arsipkan</button>
          )}
        </div>
      </div>
      ...
      {canPrepare && (
        <div className="mb-6 p-4 bg-cyan-500/10 border border-cyan-500/20 rounded-xl">
          <div className="text-cyan-600 dark:text-cyan-400 font-medium text-sm">Langkah 1: Menyiapkan Dokumen</div>
          <div className="text-cyan-700 dark:text-cyan-300/70 text-sm mt-1">Review dokumen dan file lampiran, lalu klik "Siapkan Dokumen" untuk menandai siap diarsipkan.</div>
        </div>
      )}

      {canArchive && (
        <div className="mb-6 p-4 bg-green-500/10 border border-green-500/20 rounded-xl">
          <div className="text-green-650 dark:text-green-400 font-medium text-sm">Langkah 2: Mengarsipkan Dokumen</div>
          <div className="text-green-750 dark:text-green-300/70 text-sm mt-1">Dokumen sudah disiapkan. Klik "Arsipkan" untuk mengarsipkan dokumen secara final.</div>
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          {document.reports?.length > 0 && (
            <div className="bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 rounded-2xl p-6 shadow-sm dark:shadow-none">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Laporan Terkait</h2>
              <div className="space-y-2">
                {document.reports.map(report => (
                  <div key={report.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-transparent">
                    <div>
                      <div className="text-gray-900 dark:text-white text-sm">Laporan #{report.id}</div>
                      <div className="text-gray-500 dark:text-gray-500 text-xs">Oleh: {report.creator?.name} &middot; {new Date(report.created_at).toLocaleDateString('id-ID')}</div>
                    </div>
        ...
        <div>
          <div className="bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 rounded-2xl p-6 shadow-sm dark:shadow-none">
            <div className="flex justify-between items-center mb-4">
              ...
              <Link href={`/admin/documents/${document.id}/audit-log`} className="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300">Detail</Link>
            </div>
