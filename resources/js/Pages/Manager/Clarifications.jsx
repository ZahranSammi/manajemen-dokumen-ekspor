...
export default function Clarifications({ clarifications }) {
  return (
    <Layout title="Klarifikasi Perlu Ditinjau">
      <h1 className="text-2xl font-semibold text-gray-900 dark:text-white mb-6">Klarifikasi Perlu Ditinjau</h1>
      <div className="bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 rounded-2xl p-6 shadow-sm dark:shadow-none">
        {!clarifications?.data?.length ? (
          <div className="text-center text-gray-500 py-12">Tidak ada klarifikasi yang perlu ditinjau.</div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400 text-left">
                  ...
                  <th className="pb-3 font-medium">Jawaban Supplier</th>
              ...
              <tbody>
                {clarifications.data.map(c => (
                  <tr key={c.id} className="border-b border-gray-100 dark:border-white/5 hover:bg-gray-100/50 dark:hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="py-3 text-gray-600 dark:text-gray-400 max-w-xs truncate">{c.answer || '-'}</td>
                    <td className="py-3">
                      <Link href={`/manager/clarifications/${c.id}`} className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 text-sm">Detail</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
